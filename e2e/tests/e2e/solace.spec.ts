...
test("Checkout", async ({ page }) => {
  ...
  const ContinueToDeliveryButton = await page
    .getByText("Proceed to payment")
    .first();
...
test("load footer links", async ({ page }) => {
  await page.goto(BaseUrl);
  const AboutUsLink = await page
    .getByTestId("payment-and-pricing-link")
    .first();
...
test("edit account details", async ({ page }) => {
  ...
  const AccountDetailsButton = await page
    .getByTestId("account-settings-nav-item")
    .nth(1);
